...
//[{"taskinfo":"测试是的 的的的的","workingtime":8,"numbers":2},{"taskinfo":"测试是的 的的的的","workingtime":8,"numbers":2}]
    ...
    @ApiOperation(value = "根据会议id得到会议详细信息",notes = "参数： <br>1、会议ID meetingId   " +
            "<br>如果登陆(带有token情况下)，返回值中relations属性下是自己与此会议的关系类型" +
            "（类型：1 创建的会议；2 参加的会议；3 收藏的会议 ；4  志愿者申请中 ；5 志愿者； 6 申请志愿者失败）" )
    @ResponseBody
    ...
    public ResultBean getMeetingDetail(@PathVariable("meetingId") int id,HttpServletRequest request) {
        ...
        if (userId != null){
            types = userMeetingService.types((Integer) userId, id);
        }
...
//    分页获取首页会议1
    ...
    @ApiOperation(value = "获取根据开始时间排序后的数据（最近的）",notes = "参数: 1.起始位置 offset 2. 条数 limit")
    @ResponseBody
    @RequestMapping(path = "/meetings/orderByStart", method = RequestMethod.GET)
    public ResultBean findMeetingsOrderByStartLimit(int offset, int limit) {
        List<Meeting> meetings = meetingService.findMeetingsOrderByStartLimit(offset, limit);
        HashMap hashMap = new HashMap();
        hashMap.put("meetings", meetings);
        return ResultBean.success(hashMap);
    }
    ...
    @ApiOperation(value = "根据条件查询会议",notes = "参数： 此处参数为独立参数，若要使用json串方式请使用dy/string <br><br>1、 type 会议类型 如 1 (科研)  " +
...
//        int meetingsType = meetingService.findMeetingsType(type);
    ...
    @ApiOperation(value = "根据条件查询会议",notes = "参数： 编码后Json串名：type 其中可选属性有<br>1、 type 会议类型 如 1 (科研)  " +
            " <br>2、location 会议地点 如逸夫楼 <br>3、date 会议日期 格式 yyyy-MM-dd  如2019-11-25 ")
    @ResponseBody
    @RequestMapping(path = "/meetings/dy/string", method = RequestMethod.GET)
    public ResultBean findMeetingsDyString(String type) throws UnsupportedEncodingException, JsonProcessingException {
//        int meetingsType = meetingService.findMeetingsType(type);
        Object o = jsonUtil.decodeUTF8JsonToObject(type, Meeting.class);
        Meeting meeting = (Meeting)o;
        List<Meeting> meetings = meetingService.findMeetingsDy(meeting.getTypeid(), meeting.getLocation(), meeting.getCloseTime(),meeting.getStartTime(), meeting.getmName());
        HashMap hashMap = new HashMap();
        hashMap.put("meetings", meetings);
        return ResultBean.success(hashMap);
    }
